 ...
 *
 * If you have other requirements, you could have a look at Streamflyer (https://code.google.com/p/streamflyer/).
 */
public class StreamRegex {
    ...
    public StreamRegex(String pattern) {
        ...
        readPipe(root, new int[]{0,0});    // {pattern-pos, group-count}
        preprocess();
    }
    
    /** this will hungrily match as much as possible - it will return null on a partial match and never backtrack! */
    ...
    private boolean matches(FeatureInputStream in, SRNode start) throws IOException {
        ...
        for(int y = 0;; y++) {
            ...
            if (!pruneLanes(lanes)) {return false;}
            if (!recycleLanes(y, lanes)) {return in.peekChar() == -1;}
        }
    }
    Matcher backToGroup(Matcher m) {
        m = m.prev;
        while(m != null) {
            if (m.node.GROUP) return m;
            m = m.prev;
        }
        return null;
    }
    ...
    private boolean readPipe(SRNode x, int[] ii) {  // foo|bar|(baz)
        while(ii[0] < pattern.length()) {
            readSeq(x, ii);
            if (!readChar('|', ii, NOOP)) {break;}
        }
        return true;
    }
    private boolean readSeq(SRNode x, int[] ii) {    // fo[o0](bar){0,1}
        while(ii[0] < pattern.length()) {
            if      (peekChar(')', ii)) {return true;}
            else if (peekChar('|', ii)) {return true;}
            boolean ok = 
    ...
    private boolean peekChar(int x, int[] ii) { return x == pattern.charAt(ii[0]); }
    private boolean readChar(int match, int[] ii, int[] ret) {
        if (ii[0] >= pattern.length()) {return false;}
        int c = pattern.charAt(ii[0]);
        if (match != -1 && match != c) {return false;}
        if (')' == c && --ii[1] < 0)
    ...
    private boolean readGroup(SRNode x, int[] ii) {
        if (!readChar('(', ii, NOOP)) {return false;}
        final int offset = ii[0];
        ii[1]++;
        if (x.GROUP || x.x != null) {
        ...
        x.next = null;
        if (!readChar(')', ii, NOOP))
            throwEx("unterminated group ", new int[]{start});
        return true;
    }
    private boolean readCharClass(SRNode x, int[] ii) {
        ...
        for (;;) {
            ...
            if      (']' == b)          {break;}
            else if (NEGATION == b)     {negation= !negation;  continue;}
            ...
            if (pattern.length() == ii[0])
                throwEx("unterminated character class ", new int[]{offset});
            a= b;
        }
        cc.simplify();
    ...
    private boolean readCount(SRNode x, int[] ii) {
        int before = ii[0];
        if (readChar('?', ii, NOOP)) return setCount(x, 0, 1, ii);
        if (readChar('*', ii, NOOP)) return setCount(x, 0, Integer.MAX_VALUE, ii);
        if (readChar('+', ii, NOOP)) return setCount(x, 1, Integer.MAX_VALUE, ii);
        if (readChar('{', ii, NOOP)) {
            int min = 0, max = Integer.MAX_VALUE;
            int start = ii[0];
            while(Character.isDigit(pattern.charAt(ii[0]))) ii[0]++;
            if (ii[0] > start) { 
            ...
            if (!readChar('}', ii, NOOP)) {
                throwEx("  } is missing", new int[]{before});
            }
    ...
    static private final int[] NOOP = new int[]{0};
    ...
    private int nextChar(int[] ii, BitSet ctx) {
        ...
        boolean ok = 
                CTX_CC == ctx && (
                    readCharThenSet('-', ii, cc, CHAR_RANGE)
    ...
    private boolean set(int[] cc, int x) {  cc[0]= x;  return true;  }
    private boolean readCharThenSet(int match, int[] ii, int[] ret, int set) {
        if (!readChar(match,ii, ret)) {return false;}
        ret[0] = set;  return true;  }
    ...
    private boolean readUTF(int[] ii, int[] ret) {
        if (!readChar('u', ii, ret)) {return false;}
    ...
    private boolean readCheck(char ch, int[] ii, int[] ret, BitSet check) {
        if (!check.get(ch)) {return false;}
        ret[0] = ch;  ii[0]++;  return true;  }
    ...
    private boolean preprocessPossiblyEnding(SRNode a) {
        ...
        if (!a.GROUP && a.min > 0) {return false;}  // return false for sake of previous
...
class SRNode {
    ...
    SRNode(CharClass x, SRNode prev, int offset) {
        ...
        this.offset = offset;
        SRNode it = prev;
        while(it != null && !it.GROUP) it = it.prev;
        if (it != null && it.GROUP) {grouppar = it;}
        it = prev;
        for(; it != null; nestingLevel++) { it = it.prev; }
    }
    ...
    static void print_(PrintStream out, SRNode n, int ind) {
        out.print(bff.Arf.repeat(' ', ind));
        out.println(n.toString());
        if (n.any != null) {
            //out.println(new String(f.Arf.repeat(' ', ind)) + "ANY:");
            for(SRNode c : n.any)
                print_(out, c, ind+2);
            out.println(bff.Arf.repeat('-', ind+4));
        }
        if (n.next != null)
...
class CharRange {
    ...
    public String toString() { return (char)start+"-"+(char)end; }
...
class CharClass {
    ...
    BitSet in() { if (in == null) { in = new BitSet(2048); }  return in; }
    ...
    void simplify() {
        if (in != null) {
            ...
            for(int i = 0; i < out.length(); i++)
                if (out.get(i)) {in.clear(i);}
            out = null;
        }
        if (inr.size() == 0) {inr = null;}
        if (outr.size() == 0) {outr = null;}
    }
    boolean in(int x) {
        return this.x == x ||
                   this.x == -1 
    ...
    @Override
    public boolean equals(Object obj) {
        if (!(obj instanceof CharClass)) return false;
        CharClass c = (CharClass)obj;
        if (x >= 0 || c.x >= 0) return x == c.x;
...
class Matcher {
    ...
    Matcher prev_(Matcher prev) { this.prev = prev;  return this; }
}
